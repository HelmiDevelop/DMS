...
public class PackagingJpaController extends AbstractController<Packaging> implements Serializable {
    ...
    private List<Packaging> findPackagingEntities(boolean all, int maxResults, int firstResult) {
        ...
        try {
            List<Packaging> packagings = em.createNativeQuery("SELECT pack FROM Packaging pack", Packaging.class).getResultList();
            return packagings;
        } finally {
            em.close();
        }
    ...
    public int getPackagingCount() {
        EntityManager em = getEntityManager();
        try {
            int count = (int) em.createNativeQuery("SELECT COUNT(pack) FROM Packaging pack", int.class).getSingleResult();
            return count;
        } finally {
            em.close();
        }
