...
public class SupplierJpaController extends AbstractController<Supplier> implements Serializable {
    ...
    private List<Supplier> findSupplierEntities(boolean all, int maxResults, int firstResult) {
        ...
        try {
            List<Supplier> suppliers = em.createNativeQuery("SELECT sup FROM Supplier sup", Supplier.class).getResultList();
            return suppliers;
        } finally {
            em.close();
        }
    ...
    public int getSupplierCount() {
        EntityManager em = getEntityManager();
        try {
            int count = (int) em.createNativeQuery("SELECT COUNT(sup) FROM Supplier sup", int.class).getSingleResult();
            return count;
        } finally {
            em.close();
        }
