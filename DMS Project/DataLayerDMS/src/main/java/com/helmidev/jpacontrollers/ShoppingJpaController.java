...
public class ShoppingJpaController extends AbstractController<Shopping> implements Serializable {
    ...
    private List<Shopping> findShoppingEntities(boolean all, int maxResults, int firstResult) {
        ...
        try {
            List<Shopping> shoppings = em.createNativeQuery("SELECT shop FROM Shopping shop", Shopping.class).getResultList();
            return shoppings;
        } finally {
            em.close();
        }
    ...
    public int getShoppingCount() {
        EntityManager em = getEntityManager();
        try {
            int count = (int) em.createNativeQuery("SELECT COUNT(shop) FROM Shopping shop", int.class).getSingleResult();
            return count;
        } finally {
            em.close();
        }
